...
public:
    ...
    auto submit(F f, Args&&... args) -> std::future<decltype(f(args...))> {
        ...
        std::function<RetType()> task_func = std::bind(std::forward<F>(f), std::forward<Args>(args)...); 
    
        auto task_package_ptr = std::make_shared<std::packaged_task<RetType()>>(task_func);
...

        {
            ...
            task_queue.emplace(wrapper_func, DEFAULT_PRIOR_LEVEL);
        }
    ...
    auto submit(int prior_level, F f, Args&&... args) -> std::future<decltype(f(args...))> {
        ...
        {
            std::unique_lock<std::mutex> q_lck(task_queue_mtx);
            task_queue.emplace(wrapper_func, prior_level);
        }
